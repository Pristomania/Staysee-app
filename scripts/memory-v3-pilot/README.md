...
The paid six-category `hypothesis-admission-r3` run completed 6/6 sequential requests without retry and matched all 6/6 required items plus 2/2 acceptable items. Structural overall precision/recall/F1 were `0.8889 / 1 / 0.9412`; evidence precision/recall/F1 were `0.8333 / 1 / 0.9091`. `safety-03` stayed empty. In `recurrence-02`, the only unlisted item was an extra hypothesis: the user explicitly named shame, so that meaning should have remained a recurrence qualifier instead of becoming a separate hypothesis. This run is a strong offline-benchmark result, not production-readiness evidence; semantic and forbidden-meaning review remain human judgments, and actual usage/cost remain unknown.
