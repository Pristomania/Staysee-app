...
export interface DialogueHistorySourceSnapshotInput {
  userId: string;
  sourceCutoff: string;
  conversations: DialogueHistoryConversationInput[];
}

export interface DialogueHistoryPreparedChunk {
  chunkId: string;
...
export interface PreparedDialogueHistoryBackfill {
  userId: string;
...
function makeError(token: object): Error {
  const error = new Error('[memory-v3:dialogue-history-backfill-contract] source is invalid');
  error.name = 'MemoryV3DialogueHistoryBackfillContractError';
...
export function canonicalDialogueHistoryDigest(value: unknown): string {
...
function validateAndCloneSourceSnapshot(
  token: object,
  value: unknown,
): DialogueHistorySourceSnapshotInput {
  ...
  return {
    ...
    sourceCutoff,
    conversations,
  } as DialogueHistorySourceSnapshotInput;
}
...
function chunkOneConversation(input: {
  ...
  conversationOrdinal: number;
  token: object;
}): DialogueHistoryPreparedChunk[] {
  const { profile, source, conversation, conversationOrdinal, token } = input;
  const chunks: DialogueHistoryPreparedChunk[] = [];
  let cursor = 0;
...
function comparePreparedChunks(
  left: DialogueHistoryPreparedChunk,
  right: DialogueHistoryPreparedChunk,
): number {
...
function projectManifest(
  ...
  conversations: DialogueHistoryConversationInput[],
  chunks: DialogueHistoryPreparedChunk[],
): DialogueHistoryBackfillManifest {
...
export function prepareDialogueHistoryBackfill(input: {
  profileId: unknown;
  snapshot: unknown;
