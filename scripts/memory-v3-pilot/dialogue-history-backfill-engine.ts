...
import {
  ...
  type DialogueHistoryBackfillManifest,
  type DialogueHistoryPreparedChunk,
...
import { buildMemoryV3DialogueReconcileRequest } from '../../supabase/functions/_shared/memoryV3/dialoguePrompt.ts';
import type {
  MemoryV3DialogueModelAdapter,
  MemoryV3DialogueTransportResult,
} from '../../supabase/functions/_shared/memoryV3/dialogueTransport.ts';
import {
  applyMemoryV3DialogueStep,
  createEmptyMemoryV3DialogueState,
} from '../../supabase/functions/_shared/memoryV3/dialogueReducer.ts';

export type DialogueHistoryBackfillStage =
  | 'extractor_transport'
  | 'extractor_parse'
  | 'extractor_contract'
  | 'reconciler_request'
...
type ReviewPacket = {
  schemaVersion: 'memory-v3-dialogue-history-review-packet-v1';
...
const OPTIONS_FIELDS = [
  ...
  'conversationRevisions', 'extractorAdapter', 'reconcilerAdapter',
] as const;
...
const ERROR_DETAILS = new WeakMap<object, { stage: DialogueHistoryBackfillStage | null; code: string }>();
const AUTHENTIC_RESULTS = new WeakSet<object>();

function createEngineError(stage: DialogueHistoryBackfillStage | null, code: string): Error {
  const error = new Error('[memory-v3:dialogue-history-backfill-engine] operation failed');
  error.name = 'MemoryV3DialogueHistoryBackfillEngineError';
  OWN_ERRORS.add(error);
  ERROR_DETAILS.set(error, { stage, code });
  return error;
}

function fail(stage: DialogueHistoryBackfillStage | null, code: string): never {
  throw createEngineError(stage, code);
}

function details(error: unknown): { stage: DialogueHistoryBackfillStage | null; code: string } | null {
  return typeof error === 'object' && error !== null && OWN_ERRORS.has(error)
    ? ERROR_DETAILS.get(error) ?? null
    : null;
}

function safeKeys(value: object, stage: DialogueHistoryBackfillStage | null): PropertyKey[] {
  try {
...
function strictRecord(
  ...
  stage: DialogueHistoryBackfillStage | null,
  code: string,
): JsonRecord {
...
function validatePreparedUnsafe(value: unknown, profile: LifecycleHistoryBackfillProfile): PreparedDialogueHistoryBackfill {
  ...
  if (manifestRecord.schemaVersion !== 'memory-v3-dialogue-history-manifest-v1' ||
  ...
  ) as unknown as PreparedDialogueHistoryBackfill['chunks'];
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    if (!chunk || !projected || chunkIds.has(chunk.chunkId) ||
      ...
      chunk.sourceDigest !== canonicalDialogueHistoryDigest({
        conversationId: chunk.conversationId,
        messages: dialogue.messages,
      }) ||
      chunk.chunkId !== canonicalDialogueHistoryDigest([
  ...
  const manifest: DialogueHistoryBackfillManifest = {
...
type ExtractorTransport = MemoryV3TransportResult & {
  resolvedModel: DialogueHistoryResolvedModel;
};
type ReconcilerTransport = MemoryV3DialogueTransportResult & {
  resolvedModel: DialogueHistoryResolvedModel;
};
...
interface DialogueHistoryProviderCallGate {
  call<T>(stage: 'extractor_transport' | 'reconciler_transport', inner: () => Promise<T>): Promise<T>;
  getAttemptCount(): number;
  isOwnCapError(error: unknown): boolean;
}

function createDialogueHistoryProviderCallGate(maxRequests: number): DialogueHistoryProviderCallGate {
...
export function __testOnlyCreateDialogueHistoryProviderCallGate(maxRequests: number): {
  callExtractor<T>(inner: () => Promise<T>): Promise<T>;
  callReconciler<T>(inner: () => Promise<T>): Promise<T>;
  getAttemptCount(): number;
} {
  const gate = createDialogueHistoryProviderCallGate(maxRequests);
  return {
...
function buildBaseResult(
  ...
  maxRequests: number,
  execute: boolean,
): Omit<DialogueHistoryBackfillResult,
  'conversations' | 'providerCallCount' | 'actualUsage' | 'actualCostUsd'> {
  return {
    schemaVersion: 'memory-v3-dialogue-history-result-v1',
    profileId: 'memory-v3-dialogue-history-backfill-v1',
    model: profile.model,
...
export async function runDialogueHistoryBackfill(input: {
  profileId: unknown;
  prepared: unknown;
  ...
  nowMs: number;
  execute: unknown;
  conversationRevisions: ReadonlyMap<string, number>;
  extractorAdapter?: ExtractorAdapter;
  reconcilerAdapter?: ReconcilerAdapter;
}): Promise<DialogueHistoryBackfillResult> {
  ...
  if (!(root.conversationRevisions instanceof Map)) fail(null, 'conversation_revisions_invalid');
  const conversationRevisions = root.conversationRevisions as ReadonlyMap<string, number>;
  ...
  if (root.execute === false) {
    const result = deepFreeze<DialogueHistoryBackfillResult>({
      ...base,
      conversations: [],
      providerCallCount: 0,
      actualUsage: null,
  ...
  const providerCallGate = createDialogueHistoryProviderCallGate(budget.maxRequests);
  const usages: Usage[] = [];
  ...
  const callReconcilerOnce = async (request: Parameters<MemoryV3DialogueModelAdapter>[0]) => {
    let raw: unknown;
    try {
  ...
  // Chunks arrive as ONE global, cross-conversation array sorted by real-world
  // chronological order (see dialogue-history-backfill-contract.ts). Group them
  // by conversationOrdinal first so each conversation replays against its own
  // fresh state, independent of how its chunks are interleaved with any other
  // conversation's chunks in that global array.
  ...
  for (const conversationOrdinal of conversationOrdinals) {
    ...
    for (const chunk of chunksForConversation) {
      ...
      try {
        ...
        let extraction;
        try {
          // scopeMode is 'conversation' (not lifecycle's 'cross_conversation'): this
          // engine replays exactly one conversation's own messages per iteration, and
          // dialogueContract.ts's validateExtraction requires item.scope === 'conversation'
          // with a matching conversationId for the reconcile request to validate below.
        ...
        let reconcileBundle;
        try {
          reconcileBundle = buildMemoryV3DialogueReconcileRequest({
        ...
        let proposal;
        try {
          proposal = validateMemoryV3DialogueProposal(parsedProposal, {
            state,
            extraction,
        ...
        try {
          ...
          state = validateMemoryV3DialogueState({
            ...reduced.state,
            stateRevision: state.stateRevision + (reduced.changed ? 1 : 0),
          }, prepared.userId, conversationId);
        } catch {
  ...
  let actualUsage: DialogueHistoryBackfillResult['actualUsage'] = null;
...
export function buildDialogueHistoryReviewPacket(result: unknown): ReviewPacket {
  if (typeof result !== 'object' || result === null || !AUTHENTIC_RESULTS.has(result)) {
    fail(null, 'review_result_invalid');
  }
  const benchmarkResult = result as DialogueHistoryBackfillResult;
  if (!benchmarkResult.execute) {
    fail(null, 'review_result_invalid');
  }
  for (const conversation of benchmarkResult.conversations) {
    const manifestConversation = benchmarkResult.manifest.conversations.find(
      (row) => row.conversationOrdinal === conversation.conversationOrdinal,
    );
    if (
      conversation.failureCount !== 0 ||
      conversation.finalState === null ||
      !manifestConversation ||
      conversation.successChunkCount !== manifestConversation.chunkCount
    ) {
      fail(null, 'review_result_invalid');
    }
  }
  const items = benchmarkResult.conversations.flatMap((conversation) =>
