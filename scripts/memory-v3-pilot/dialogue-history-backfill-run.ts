/** Import-safe filesystem and process composition root for the Memory V3
 * dialogue-scope historical backfill. */
...
const PREFIX = '[memory-v3:dialogue-history-backfill-run]';
const ERROR_NAME = 'MemoryV3DialogueHistoryBackfillRunError';
...
const REQUIRED_FIELDS = [
  ...
  'createSourceAndRevisionAccess', 'fetchImpl', 'nowMs', 'writeStdout', 'writeStderr',
] as const;
...
function projectSummary(
  payload: Awaited<ReturnType<typeof runDialogueHistoryBackfillFromArgv>>,
  ...
  for (const conversation of result.conversations) {
    const items = conversation.finalState?.items ?? [];
...
export async function main(input: {
  ...
  linkImpl: (existingPath: string, newPath: string) => Promise<void>;
  unlinkImpl: (path: string) => Promise<void>;
  createSourceAndRevisionAccess: (url: string, serviceKey: string) => {
  ...
  try {
    ...
    // One real Supabase client backs BOTH the source reader and the
    // revision-client input below. `createSourceAndRevisionAccess` is called
    // at most once per run and its result is cached here; the two closures
    // handed to runDialogueHistoryBackfillFromArgv (`sourceReader` and
    // `revisionClient`) both read from that single cached result, never
    // opening a second connection.
    let combinedAccess: { reader: LifecycleHistorySourceReader; revisionClient: unknown } | null = null;
    const getCombinedAccess = (url: string, serviceKey: string) => {
      combinedAccess ??= (root.createSourceAndRevisionAccess as
        (url: string, serviceKey: string) =>
          { reader: LifecycleHistorySourceReader; revisionClient: unknown })(url, serviceKey);
      return combinedAccess;
    };

    const payload = await runDialogueHistoryBackfillFromArgv({
      argv: parsed.argv,
      sourceReader: (url: string, serviceKey: string) => getCombinedAccess(url, serviceKey).reader,
      revisionClient: (url: string, serviceKey: string) => getCombinedAccess(url, serviceKey).revisionClient,
      readEnvText,
...
async function runDirect() {
  ...
  const exitCode = await main({
    ...
    linkImpl: link,
    unlinkImpl: unlink,
    createSourceAndRevisionAccess: (url, serviceKey) => {
      const client = createClient(url, serviceKey, {
        auth: { persistSession: false, autoRefreshToken: false, detectSessionInUrl: false },
      });
      return { reader: createLifecycleHistorySupabaseReader(client), revisionClient: client };
    },
