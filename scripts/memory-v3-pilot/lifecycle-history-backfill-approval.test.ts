...
import { generateApprovalDecision } from "./lifecycle-history-backfill-approval.ts";

const execFileAsync = promisify(execFile);
const APPROVAL_PATH = new URL('./lifecycle-history-backfill-approval.ts', import.meta.url);
...
function successfulArtifact() {
  return {
    benchmarkResult: {
      execute: true,
      failureCount: 0,
      finalState: { items: [{ memoryKey: "a".repeat(64) }, { memoryKey: "b".repeat(64) }] },
    },
    semanticReviewPacket: { payloadSha256: "d".repeat(64) },
  };
}

describe("lifecycle history backfill approval generator", () => {
  it("generates a PASS decision covering every item from the account-wide state, in order", () => {
    ...
    assert.deepEqual(decision.items.map((item) => item.memoryKey), ["a".repeat(64), "b".repeat(64)]);
  ...
  it("refuses to approve an artifact whose finalState is null", () => {
    const artifact = successfulArtifact();
    (artifact.benchmarkResult as { finalState: unknown }).finalState = null;
    assert.throws(() => generateApprovalDecision(artifact));
  });

  it("refuses to approve an artifact whose finalState has zero items", () => {
    const artifact = successfulArtifact();
    artifact.benchmarkResult.finalState.items = [];
    assert.throws(() => generateApprovalDecision(artifact));
  });
  ...
  it("correctly validates and writes review file from artifact JSON", async () => {
    const tempDir = await mkdtemp(join(tmpdir(), "lifecycle-approval-test-"));
    try {
      ...
      assert.equal(writtenDecision.schemaVersion, "memory-v3-lifecycle-history-review-v1");
      assert.deepEqual(writtenDecision.items.map((item: { memoryKey: string }) => item.memoryKey), ["a".repeat(64), "b".repeat(64)]);
  ...
  it("the direct() CLI wrapper actually runs end to end when invoked with the documented 3-token argv", async () => {
    const tempDir = await mkdtemp(join(tmpdir(), "lifecycle-approval-cli-test-"));
    try {
