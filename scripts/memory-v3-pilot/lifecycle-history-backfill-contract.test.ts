...
function captureContractError(operation: () => unknown): Error {
  ...
  assert.equal(thrown.name, 'MemoryV3LifecycleHistoryBackfillContractError');
  assert.equal(
    thrown.message,
    '[memory-v3:lifecycle-history-backfill-contract] source is invalid',
  );
...
describe('history source validation and canonicalization', () => {
  it('does not mutate source objects and returns a deeply frozen detached result', () => {
    ...
    const result = prepareLifecycleHistoryBackfill({ profileId: PROFILE_ID, snapshot: input });
  ...
  it('rejects a source row even one nanosecond after the exact cutoff', () => {
    ...
    captureContractError(() =>
      prepareLifecycleHistoryBackfill({ profileId: PROFILE_ID, snapshot: input })
    );
  });
...
describe('history digests and public manifest', () => {
  it('hashes the exact extractor request bytes and changes with the request', () => {
    ...
    const prepared = prepareLifecycleHistoryBackfill({ profileId: PROFILE_ID, snapshot: source });
  ...
  it('canonicalizes object keys but preserves array order', () => {
    assert.equal(
      canonicalLifecycleHistoryDigest({ b: 2, a: [1, 2] }),
      canonicalLifecycleHistoryDigest({ a: [1, 2], b: 2 }),
    );
    assert.notEqual(
      canonicalLifecycleHistoryDigest({ a: [1, 2], b: 2 }),
      canonicalLifecycleHistoryDigest({ a: [2, 1], b: 2 }),
    );
  });
...
describe('history source hostile input boundary', () => {
  it('rejects getters, setters, symbols, non-enumerable fields, sparse arrays, and cycles', () => {
    ...
    captureContractError(() => prepareLifecycleHistoryBackfill(getterInput as never));
    ...
    captureContractError(() => prepareLifecycleHistoryBackfill(setterInput as never));
    ...
    captureContractError(() => prepareLifecycleHistoryBackfill(symbolInput));
    ...
    captureContractError(() => prepareLifecycleHistoryBackfill(nonEnumerable as never));

    const sparse = snapshot([conversation(1, [message(1)])]);
    sparse.conversations.length = 2;
    captureContractError(() =>
      prepareLifecycleHistoryBackfill({ profileId: PROFILE_ID, snapshot: sparse })
    );

    const cyclic = snapshot([conversation(1, [message(1)])]) as Record<string, unknown>;
    cyclic.sourceCutoff = cyclic;
    captureContractError(() =>
      prepareLifecycleHistoryBackfill({ profileId: PROFILE_ID, snapshot: cyclic })
    );
  });
  ...
  it('rejects transparent, throwing, and revoked proxies without executing traps or leaking', () => {
    ...
    captureContractError(() =>
      prepareLifecycleHistoryBackfill({ profileId: PROFILE_ID, snapshot: transparent })
    );
    ...
    const error = captureContractError(() =>
      prepareLifecycleHistoryBackfill({ profileId: PROFILE_ID, snapshot: throwing })
    );
    ...
    revoked.revoke();
    captureContractError(() =>
      prepareLifecycleHistoryBackfill({ profileId: PROFILE_ID, snapshot: revoked.proxy })
    );
  });
});
