...
function reconcilerRequest(): MemoryV3LifecycleReconcileRequest {
  return {
...
describe('history-only OpenRouter model route', () => {
  it('routes extractor once, preserves privacy, reads the response once, and records fallback', async () => {
    ...
    const { extractorAdapter } = createLifecycleHistoryRoutedAdapters({ apiKey: API_KEY, fetchImpl });
  ...
  it('routes reconciler once, preserves privacy, reads the response once, and records fallback', async () => {
    ...
    const fetchImpl = recordingFetch(() => response);
    const { reconcilerAdapter } = createLifecycleHistoryRoutedAdapters({ apiKey: API_KEY, fetchImpl });
  ...
  it('matches existing request bodies except for the deterministic model-to-models substitution', async () => {
    ...
    const lifecycleFetch = recordingFetch(() => new Response(JSON.stringify(
      providerBody(LIFECYCLE_HISTORY_PRIMARY_MODEL, RECONCILED),
    ), { status: 200 }));
    await createMemoryV3LifecycleOpenRouterAdapter({ apiKey: API_KEY, fetchImpl: lifecycleFetch })(
      reconcilerRequest(),
    );
    const lifecycleBaseline = routedBody(lifecycleFetch);

    const routedExtractorFetch = recordingFetch(() => trackedResponse(
      providerBody(LIFECYCLE_HISTORY_PRIMARY_MODEL, EXTRACTED),
    ));
    const routedLifecycleFetch = recordingFetch(() => trackedResponse(
      providerBody(LIFECYCLE_HISTORY_PRIMARY_MODEL, RECONCILED),
    ));
    await createLifecycleHistoryRoutedAdapters({ apiKey: API_KEY, fetchImpl: routedExtractorFetch })
      .extractorAdapter(extractorRequest());
    await createLifecycleHistoryRoutedAdapters({ apiKey: API_KEY, fetchImpl: routedLifecycleFetch })
      .reconcilerAdapter(reconcilerRequest());

    for (const [baseline, actual] of [
      [extractorBaseline!, routedBody(routedExtractorFetch)],
      [lifecycleBaseline, routedBody(routedLifecycleFetch)],
    ] as const) {
  ...
  it('does not read an HTTP error body and does not retry a rejected inner fetch', async () => {
    ...
    await captureError(() => createLifecycleHistoryRoutedAdapters({ apiKey: API_KEY, fetchImpl: httpFetch })
      .extractorAdapter(extractorRequest()));
    ...
    await captureError(() => createLifecycleHistoryRoutedAdapters({ apiKey: API_KEY, fetchImpl: rejectingFetch })
      .reconcilerAdapter(reconcilerRequest()));
  ...
  it('rejects hostile configuration without executing getters or calling fetch', () => {
    ...
    assert.throws(
      () => createLifecycleHistoryRoutedAdapters(hostile as never),
      /^MemoryV3LifecycleHistoryProviderError:/,
    );
    assert.equal(getterCalls, 0);
...
describe('history provider isolation', () => {
  it('locks exact imports and keeps frozen transports byte-identical', async () => {
    const sourcePath = fileURLToPath(new URL('./lifecycle-history-backfill-provider.ts', import.meta.url));
    const source = await readFile(sourcePath, 'utf8');
    const imports = [...source.matchAll(/\b(?:from\s*)?["']([^"']+)["']/g)]
      .map((match) => match[1])
