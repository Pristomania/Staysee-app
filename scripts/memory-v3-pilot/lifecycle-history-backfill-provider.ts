...
export type LifecycleHistoryReconcilerAdapter = (
...
function makeError(): Error {
  const error = new Error('[memory-v3:lifecycle-history-provider] value is invalid');
  error.name = 'MemoryV3LifecycleHistoryProviderError';
...
): LifecycleHistoryExtractorAdapter {
...
): LifecycleHistoryReconcilerAdapter {
  return async (request) => {
    observer.begin();
    try {
      const result = await adapter(request);
...
export function createLifecycleHistoryRoutedAdapters(input: {
  ...
  reconcilerAdapter: LifecycleHistoryReconcilerAdapter;
} {
  return boundary(() => {
    ...
    const reconcilerBase = createMemoryV3LifecycleOpenRouterAdapter({
      apiKey: config.apiKey,
