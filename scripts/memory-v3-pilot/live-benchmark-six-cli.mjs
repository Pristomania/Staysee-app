/**
 * Memory V3 six-case live-benchmark CLI boundary.
 * Injected dataset, fetchImpl, and readEnvText. No fs, process.env, or global fetch.
 */

import { runSixCaseLiveBenchmark } from './live-benchmark-six.mjs';

const ALLOWED_MODEL = 'openai/gpt-5.6-luna';
const ALLOWED_MAX_BUDGET_USD = '0.032';
const ALLOWED_EXTRACTOR_VERSION = 'memory-v3-openrouter-luna-six-v1';
...
function fail(message) {
  const error = new Error(`[memory-v3:live-benchmark-six-cli] ${message}`);
  error.name = 'MemoryV3SixCaseBenchmarkCliError';
...
export async function runSixCaseBenchmarkFromArgv(options) {
  ...
  if (!parsed.execute) {
    try {
      return await runSixCaseLiveBenchmark({
        ...shared,
        execute: false,
      });
    } catch (error) {
