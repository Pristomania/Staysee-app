/**
 * Memory V3 six-case composition-root tests.
 * Injected readFile/fetch/stdio only. No live OpenRouter, real .env, or paid calls.
 * Run: node --test scripts/memory-v3-pilot/live-benchmark-six-run.test.mjs
 */
...
import { main } from './live-benchmark-six-run.mjs';

const MODEL = 'openai/gpt-5.6-luna';
const API_KEY = 'test-memory-v3-six-run-key';
...
const GOLDEN_NAME = 'memory-v3-ru-golden.v1.json';

const SENTINELS = Object.freeze({
  key: API_KEY,
  env: 'RAW_SIX_RUN_ENV_SECRET_SENTINEL',
  dataset: 'RAW_SIX_RUN_DATASET_SECRET_SENTINEL',
  fetch: 'GLOBAL_FETCH_SENTINEL',
  providerMessage: 'RAW_PROVIDER_MESSAGE_SENTINEL',
  response: 'RAW_PROVIDER_RESPONSE_SENTINEL',
  getter: 'RAW_SIX_RUN_GETTER_SENTINEL',
});

const REAL_GOLDEN_TEXT = readFileSync(new URL('./memory-v3-ru-golden.v1.json', import.meta.url), 'utf8');
...
async function assertSafeFailure(fn, { readFileImpl, fetchImpl, writeStdout, writeStderr }) {
  await assert.rejects(fn, (error) => {
    assert.match(String(error.message), /^\[memory-v3:live-benchmark-six-run]/);
...
describe('live-benchmark-six-run import safety', () => {
...
describe('live-benchmark-six-run dry-run', () => {
  it('loads golden once, skips env/fetch, and prints one safe JSON plan', async () => {
    ...
    assert.deepEqual(parsed.benchmarkResult.caseIds, [...SIX_CASE_BENCHMARK_CASE_IDS]);
...
describe('live-benchmark-six-run fake execute', () => {
...
describe('live-benchmark-six-run failure privacy', () => {
  it('keeps dataset, env, fetch, and spoofed errors out of public output', async () => {
    ...
    await assert.rejects(
      ...
      /\[memory-v3:live-benchmark-six-run]/,
    );
    assert.equal(getterCalls, 0);
  });
});

describe('live-benchmark-six-run direct dry-run command', () => {
  it('runs the executable dry-run without env, network, or execute', () => {
    const script = fileURLToPath(new URL('./live-benchmark-six-run.mjs', import.meta.url));
