/**
 * Memory V3 six-case live-benchmark harness.
 * Offline fake-fetch first. No env, fs writes, global fetch fallback, retry, or CLI.
 */

import { validateCase } from './contracts.mjs';
import { isAllowlistedExtractorDiagnosticCode } from './extractor-core.mjs';
import { evaluateCase, evaluateDataset } from './evaluator.mjs';
...
export const SIX_CASE_BENCHMARK_CASE_IDS = Object.freeze([
  'memv3-ru-event-03',
...
function fail(message) {
  const error = new Error(`[memory-v3:live-benchmark-six] ${message}`);
  error.name = 'MemoryV3SixCaseBenchmarkError';
...
function selectSixCases(dataset) {
  ...
  const selected = [];
  for (const caseId of SIX_CASE_BENCHMARK_CASE_IDS) {
    const matches = [];
...
function projectGold(caseData) {
  const gold = [];
  for (const [kind, listName] of Object.entries(GOLD_LIST_BY_KIND)) {
    const list = caseData.gold[listName];
    for (const entry of list) {
      gold.push({
        ...
        supportMessageIds: [...(entry.supportMessageIds ?? [])],
        correctedMessageIds: [...(entry.correctedMessageIds ?? [])],
        contradictedMessageIds: [...(entry.contradictedMessageIds ?? [])],
        rejectedMessageIds: [...(entry.rejectedMessageIds ?? [])],
      });
    }
  }
  return gold;
...
function projectHarnessFailureDiagnostic(failed, recorder) {
  ...
  if (stage === 'contract' || stage === 'shape' || stage === 'parse') {
    ...
    if (
      ...
      typeof desc.set !== 'function' &&
      Object.prototype.hasOwnProperty.call(desc, 'value') &&
      // Allowlisted is a permitted string only; WeakSet + projector prove origin.
      isAllowlistedExtractorDiagnosticCode(desc.value)
    ) {
      return desc.value;
    }
    if (stage === 'parse') return 'extractor_parse_invalid';
    if (stage === 'shape') return 'extractor_shape_invalid';
...
export async function runSixCaseLiveBenchmark(options) {
  ...
  for (const entry of selectedSet.selected) {
    ...
    let evaluation;
    try {
      evaluation = evaluateCase(entry.raw, run.extraction);
    } catch {
      cases.push({
        caseId: entry.caseId,
        stage: 'unknown',
  ...
  let aggregate = null;
  try {
    const datasetReport = evaluateDataset(subset, extractions, {
      extractorVersion: inspected.extractorVersion,
    });
...
function inspectAlignedIdList(value, path) {
  ...
  if (ids.length !== SIX_CASE_BENCHMARK_CASE_IDS.length) {
    throw fail(`${path} must contain exactly six case ids`);
  }
  for (let index = 0; index < ids.length; index += 1) {
    if (ids[index] !== SIX_CASE_BENCHMARK_CASE_IDS[index]) {
...
function inspectAlignedResultCases(value, path) {
  ...
  if (cases.length !== SIX_CASE_BENCHMARK_CASE_IDS.length) {
    throw fail(`${path} must contain exactly six cases`);
  }
  const seen = new Set();
  for (let index = 0; index < cases.length; index += 1) {
    const caseId = readDataCaseId(cases[index]);
    if (caseId !== SIX_CASE_BENCHMARK_CASE_IDS[index]) {
      throw fail(`${path} is misaligned`);
    }
    if (seen.has(caseId)) {
...
function inspectBenchmarkResult(value) {
  ...
  if (!isNonEmptyString(copied.extractorVersion)) {
    throw fail('benchmarkResult.extractorVersion is required');
  }
...
export function buildSixCaseSemanticReviewPacket(options) {
  if (options === null || typeof options !== 'object' || Array.isArray(options)) {
    throw fail('options must be a plain object');
  }
