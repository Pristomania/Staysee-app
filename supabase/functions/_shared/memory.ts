...
- Факты (кто, где живёт, сроки, события) — только из реплик пользователя, ПОДТВЕРЖДЁННЫХ СЛОВ и её строк в АРХИВЕ. Не из ответов StaySee.
- ПАМЯТЬ БЕСЕДЫ и СКВОЗНАЯ ПАМЯТЬ — черновик; при «помнишь» не утверждай деталь, если её нет дословно в её сообщениях ниже.
- Запрещено: «ты не говорила», если в цитатах/архиве есть её реплики про тему.
...
Не отвечай так, будто история началась только с последних реплик.
Продолжай разговор с учётом известного — не переспрашивай то, что уже есть в её словах, если только не уточняешь после паузы.
Если факта нет в её репликах/цитатах — уточни, не додумывай.
...
export function buildRecallGroundingPrompt(opts: {
  ...
  const lines = [
    ...
    "Пользователь спрашивает, что она уже говорила. Отвечай коротко.",
    "Утверждай только то, что есть в ПОДТВЕРЖДЁННЫХ СЛОВАХ или в АРХИВЕ (реплики пользователя).",
    "ПАМЯТЬ БЕСЕДЫ — ориентир по темам; не пересказывай из неё даты, быт, родственников, если нет в её цитатах.",
    ...
    "Если есть АРХИВ с её репликами — можно цитировать userText оттуда так же, как ПОДТВЕРЖДЁННЫЕ СЛОВА.",
  ];
  if (opts.evidenceCount >= 3) {
    lines.push(
      "Несколько цитат ниже по одной теме — кратко перечисли её формулировки из них; не добавляй фактов между цитатами."
    );
  } else {
  ...
  if (opts.evidenceCount === 0 && opts.archiveCount === 0) {
    lines.push(
      "Цитат и архива пользователя сейчас нет — скажи честно, что в переписке не видишь её слов про это, и попроси напомнить."
    );
  } else if (opts.evidenceCount === 0) {
    lines.push(
      "Дословных цитат нет — опирайся только на её фразы в АРХИВЕ; остальное не утверждай."
    );
  }
  return lines.join("\n");
}
...
export function applyCorrectionHints(
  ...
  if (/придумал|придумала|выдумал|выдумала|не было|не говорила|фантаз|додумал/i.test(combined)) {
    m.open_loops = dedupeItems([
      ...m.open_loops,
      "Пользователь указала на неточность памяти — факты только из её слов",
    ]);
...
- preferences: 1–2 цельных фразы про стиль общения (темп, тон, что помогает) — они пойдут в сквозную память.
...
- emotional_state / open_loops / preferences — из тона и смысла её слов.
