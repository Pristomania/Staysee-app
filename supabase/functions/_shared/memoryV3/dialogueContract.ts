...
export const MEMORY_V3_DIALOGUE_SCHEMA_VERSION = "memory-v3-dialogue-state-v1";
export const MEMORY_V3_DIALOGUE_PIPELINE_VERSION = "memory-v3-dialogue-v1";
export const MEMORY_V3_DIALOGUE_RECONCILER_VERSION = "memory-v3-dialogue-reconciler-v1";
export const MEMORY_V3_DIALOGUE_MODEL = "google/gemini-3.7-flash";
export const MEMORY_V3_DIALOGUE_MAX_SOURCE_MESSAGES = 60;
export const MEMORY_V3_DIALOGUE_MAX_CANDIDATES = 100;
export const MEMORY_V3_DIALOGUE_MAX_CANDIDATE_EVIDENCE = 500;
export const MEMORY_V3_DIALOGUE_MAX_STATE_ITEMS = 100;
export const MEMORY_V3_DIALOGUE_MAX_STATE_EVIDENCE = 500;
export const MEMORY_V3_DIALOGUE_MAX_EXTRACTOR_BYTES = 20_000;
export const MEMORY_V3_DIALOGUE_MAX_RECONCILER_BYTES = 80_000;
export const MEMORY_V3_DIALOGUE_RESERVED_INPUT_TOKENS_PER_CALL = 32_768;
export const MEMORY_V3_DIALOGUE_MAX_OUTPUT_TOKENS_PER_CALL = 1_200;
export const MEMORY_V3_DIALOGUE_MAX_MODEL_CALLS_PER_RUN = 2;
export const MEMORY_V3_DIALOGUE_MAX_DAILY_RESERVATIONS = 1;
export const MEMORY_V3_DIALOGUE_CONFIGURED_CEILING_NANODOLLARS_PER_RUN = 58_152_000;
export const MEMORY_V3_DIALOGUE_HARD_GATE_NANODOLLARS_PER_RUN = 65_000_000;

export type MemoryV3DialogueOperationType =
  | "create"
  | "confirm"
  | "revise"
  | "mark_stale"
...
export interface MemoryV3DialogueItem {
  memoryKey: string;
  kind: MemoryKind;
  claim: string;
  status: string;
...
function makeError(token: object, diagnosticCode: Diagnostic): Error {
  ...
  error.name = "MemoryV3DialogueContractError";
...
function validateEvidence(
  ...
  if (row.conversationId !== expectedConversationId || !nonEmpty(row.sourceMessageId) ||
  ...
  return { ...row } as MemoryV3DialogueEvidence;
}
...
function validateStateInternal(
  ...
  expectedConversationId?: string,
): MemoryV3DialogueState {
  const shape = "dialogue_contract_invalid_shape";
  const code = "dialogue_contract_invalid_state";
  const root = record(token, value, STATE_FIELDS, shape);
  if (root.schemaVersion !== MEMORY_V3_DIALOGUE_SCHEMA_VERSION ||
      ...
      typeof root.conversationId !== "string" || !UUID.test(root.conversationId) ||
      (expectedConversationId !== undefined &&
        (typeof expectedConversationId !== "string" || !UUID.test(expectedConversationId) ||
          root.conversationId !== expectedConversationId)) ||
  ...
  const rawItems = denseArray(token, root.items, MEMORY_V3_DIALOGUE_MAX_STATE_ITEMS, shape, code);
  let totalEvidence = 0;
  const keys = new Set<string>();
  const items = rawItems.map((raw): MemoryV3DialogueItem => {
    ...
    if (totalEvidence > MEMORY_V3_DIALOGUE_MAX_STATE_EVIDENCE || evidence.length === 0) fail(token, code);
...
function validateExtraction(token: object, value: unknown, expectedConversationId: string): MemoryV3Extraction {
  const code = "dialogue_contract_invalid_proposal";
  ...
  const items = denseArray(token, root.items, MEMORY_V3_DIALOGUE_MAX_CANDIDATES, code).map((raw) => {
    ...
    if (!nonEmpty(item.localItemKey) || !KINDS.includes(item.kind as MemoryKind) || !nonEmpty(item.claim) ||
        item.scope !== "conversation" || item.conversationId !== expectedConversationId ||
        ![...CURRENT[item.kind as MemoryKind], ...CLOSED[item.kind as MemoryKind]].includes(item.status as string) ||
        (item.sensitivity !== "normal" && item.sensitivity !== "sensitive")) fail(token, code);
    if ((item.eventTimeStart !== null && !validDateOrDateTime(item.eventTimeStart)) ||
        (item.eventTimeEnd !== null && !validDateOrDateTime(item.eventTimeEnd)) ||
  ...
  const evidence = denseArray(token, root.evidence, MEMORY_V3_DIALOGUE_MAX_CANDIDATE_EVIDENCE, code)
    .map((raw) => {
...
function validateBindings(
...
): MemoryV3DialogueReferenceBindings {
  const code = "dialogue_contract_invalid_proposal";
  const root = record(token, value, BINDINGS_FIELDS, code);
  const memories = denseArray(token, root.memories, MEMORY_V3_DIALOGUE_MAX_STATE_ITEMS, code).map((raw) => {
  ...
  const candidates = denseArray(token, root.candidates, MEMORY_V3_DIALOGUE_MAX_CANDIDATES, code).map((raw) => {
...
export function validateMemoryV3DialogueState(
  value: unknown,
  expectedUserId: string,
  expectedConversationId: string,
): MemoryV3DialogueState {
  return boundary("dialogue_contract_invalid_shape", (token) =>
    validateStateInternal(token, value, expectedUserId, expectedConversationId));
}

export function validateMemoryV3DialogueProposal(
  value: unknown,
  context: {
    state: MemoryV3DialogueState;
    extraction: MemoryV3Extraction;
  ...
  return boundary("dialogue_contract_invalid_shape", (token) => {
    ...
    const operations = denseArray(token, root.operations, MEMORY_V3_DIALOGUE_MAX_CANDIDATES, code)
    ...
    const extraction = validateExtraction(token, contextRecord.extraction, state.conversationId);
...
export function validateMemoryV3DialogueTrustedForgetKeys(
  value: unknown,
  stateValue: MemoryV3DialogueState,
): string[] {
  return boundary("dialogue_contract_invalid_shape", (token) => {
    const code = "dialogue_contract_invalid_forget";
    const state = validateStateInternal(token, stateValue);
    const existing = new Set(state.items.map((item) => item.memoryKey));
    const entries = denseArray(token, value, MEMORY_V3_DIALOGUE_MAX_STATE_ITEMS, code);
    const seen = new Set<string>();
...
export function projectSafeMemoryV3DialogueContractDiagnostic(error: unknown): Diagnostic | null {
