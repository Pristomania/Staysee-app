import {
  type MemoryV3DialogueReadContext,
  projectMemoryV3DialogueReadContext,
} from "./dialogueReadStore.ts";

export const MEMORY_V3_DIALOGUE_READ_MAX_PROMPT_BYTES = 6_000;

const OWN_ERRORS = new WeakSet<object>();
const OPEN = "[MEMORY V3 — THIS DIALOGUE'S CONTEXT]";
const CLOSE = "[/MEMORY V3 — THIS DIALOGUE'S CONTEXT]";
const RULES = [
...
function failTooLarge(): Error {
  ...
  error.name = "MemoryV3DialogueReadPromptError";
...
export function formatMemoryV3DialoguePromptBlock(
  ...
  if (new TextEncoder().encode(result).byteLength > MEMORY_V3_DIALOGUE_READ_MAX_PROMPT_BYTES) {
    throw failTooLarge();
  }
  return result;
}
