...
export interface MemoryV3DialogueReadRpcClient {
  rpc(
    name: string,
...
export interface MemoryV3DialogueReadStore {
  load(userId: string, conversationId: string): Promise<MemoryV3DialogueReadContext | null>;
}

interface InspectedClient {
  target: object;
  rpc: MemoryV3DialogueReadRpcClient["rpc"];
}
...
function fail(): Error {
  const error = new Error("[memory-v3:dialogue-read-store] operation failed");
  error.name = "MemoryV3DialogueReadStoreError";
...
export function projectMemoryV3DialogueReadContext(
  value: unknown,
): MemoryV3DialogueReadContext {
  const root = inspectRecord(value, ROOT_FIELDS);
  if (root.schemaVersion !== MEMORY_V3_DIALOGUE_READ_SCHEMA_VERSION) throw fail();
  if (
  ...
  const items = inspectArray(root.items, MEMORY_V3_DIALOGUE_READ_MAX_ITEMS).map(
    (raw) => {
  ...
  return {
    schemaVersion: MEMORY_V3_DIALOGUE_READ_SCHEMA_VERSION,
...
function inspectClient(value: unknown): InspectedClient {
  ...
  while (current !== null) {
    ...
    if (own) {
      ...
      return {
        target: value,
        rpc: own.value as MemoryV3DialogueReadRpcClient["rpc"],
      };
    }
...
export function createMemoryV3DialogueReadStore(
  clientValue: MemoryV3DialogueReadRpcClient,
): MemoryV3DialogueReadStore {
  ...
  return {
    async load(userId: string, conversationId: string) {
      ...
      if (typeof conversationId !== "string" || !UUID.test(conversationId)) throw fail();
      const data = await callRpc(client, userId, conversationId);
      return data === null ? null : projectMemoryV3DialogueReadContext(data);
    },
  };
}
