...
import {
  ...
  type MemoryV3DialogueReconcileRequest,
...
const RAW_SECRET = "RAW_DIALOGUE_PROVIDER_SECRET_SENTINEL";
...
function request(): MemoryV3DialogueReconcileRequest {
  return {
...
async function captureError(action: () => Promise<unknown>) {
  ...
  assert.equal(caught.name, "MemoryV3DialogueTransportError");
  assert.match(caught.message, /^\[memory-v3:dialogue-transport\]/);
...
describe("Memory V3 dialogue injected OpenRouter transport", () => {
  it("sends one exact privacy-locked POST and projects only content plus trusted usage", async () => {
    ...
    try {
      const result = await createMemoryV3DialogueOpenRouterAdapter({ fetchImpl, apiKey: API_KEY })(input);
    ...
    assert.deepEqual(body.response_format, {
      type: "json_schema",
      json_schema: {
        name: "memory_v3_dialogue_reconcile_response",
        strict: true,
  ...
  it("requires strict data-only configuration and never executes accessors", () => {
    ...
    for (const options of [
      ...
      assert.throws(
        () => createMemoryV3DialogueOpenRouterAdapter(options as never),
        /^MemoryV3DialogueTransportError: \[memory-v3:dialogue-transport\]/,
      );
    }
  ...
  it("rejects malformed request records before HTTP without executing getters", async () => {
    ...
    for (const invalid of [
      ...
      const fetchImpl = recordingFetch();
      await captureError(() => createMemoryV3DialogueOpenRouterAdapter({ fetchImpl, apiKey: API_KEY })(invalid as never));
  ...
  it("rejects a substituted system instruction before HTTP", async () => {
    ...
    const fetchImpl = recordingFetch();
    await captureError(() => createMemoryV3DialogueOpenRouterAdapter({ fetchImpl, apiKey: API_KEY })(input));
    assert.equal(fetchImpl.calls.length, 0);
  });
  ...
  for (const [status, diagnostic] of [
    ...
    it(`maps HTTP ${status} to ${diagnostic} without reading provider text`, async () => {
      ...
      const error = await captureError(() => createMemoryV3DialogueOpenRouterAdapter({ fetchImpl, apiKey: API_KEY })(request()));
      assert.equal(projectSafeMemoryV3DialogueTransportDiagnostic(error), diagnostic);
      assert.equal(fetchImpl.calls.length, 1);
    });
  }

  it("classifies other non-2xx statuses as invalid provider responses", async () => {
    const error = await captureError(() => createMemoryV3DialogueOpenRouterAdapter({
      fetchImpl: recordingFetch(() => new Response("", { status: 302 })),
      apiKey: API_KEY,
    })(request()));
    assert.equal(projectSafeMemoryV3DialogueTransportDiagnostic(error), "provider_response_invalid");
  });
  ...
  it("accepts nullable refusal and telemetry but returns null for untrusted usage", async () => {
    const accepted = await createMemoryV3DialogueOpenRouterAdapter({
    ...
    for (const usage of [
      ...
      const result = await createMemoryV3DialogueOpenRouterAdapter({
  ...
  it("times out a hanging body with one abort and no retry", async () => {
    ...
    try {
      ...
      const operation = createMemoryV3DialogueOpenRouterAdapter({
      ...
      assert.equal(projectSafeMemoryV3DialogueTransportDiagnostic(error), "transport_timeout");
  ...
  it("does not retry fetch rejection or trust spoofed diagnostics", async () => {
    const spoof = Object.assign(new Error(RAW_SECRET), {
      name: "MemoryV3DialogueTransportError",
    ...
    assert.equal(fetchImpl.calls.length, 1);
    assert.equal(projectSafeMemoryV3DialogueTransportDiagnostic(error), "transport_failed");
    assert.equal(projectSafeMemoryV3DialogueTransportDiagnostic(spoof), null);
  });

  it("does not trust a stolen branded error from fetch or response traps", async () => {
    const first = await captureError(() => createMemoryV3DialogueOpenRouterAdapter({
      fetchImpl: recordingFetch(() => new Response("", { status: 400 })),
      apiKey: API_KEY,
    })(request()));
    assert.equal(projectSafeMemoryV3DialogueTransportDiagnostic(first), "provider_http_400");

    const fetchImpl = recordingFetch(() => Promise.reject(first));
    const second = await captureError(() => createMemoryV3DialogueOpenRouterAdapter({ fetchImpl, apiKey: API_KEY })(request()));
    assert.notEqual(second, first);
    assert.equal(projectSafeMemoryV3DialogueTransportDiagnostic(second), "transport_failed");

    const malicious = new Proxy({}, { getPrototypeOf() { throw first; } });
    const third = await captureError(() => createMemoryV3DialogueOpenRouterAdapter({
      fetchImpl: recordingFetch(() => malicious as Response),
      apiKey: API_KEY,
    })(request()));
    assert.notEqual(third, first);
    assert.equal(projectSafeMemoryV3DialogueTransportDiagnostic(third), "provider_response_invalid");
  });
  ...
  it("rejects accessor and revoked response objects without getter execution", async () => {
    ...
    const accessorError = await captureError(() => createMemoryV3DialogueOpenRouterAdapter({
      fetchImpl: recordingFetch(() => accessor as Response),
      apiKey: API_KEY,
    })(request()));
    assert.equal(projectSafeMemoryV3DialogueTransportDiagnostic(accessorError), "provider_response_invalid");
    assert.equal(getterCalls, 0);
    ...
    revoke();
    const revoked = await captureError(() => createMemoryV3DialogueOpenRouterAdapter({
      fetchImpl: recordingFetch(() => proxy as Response),
      apiKey: API_KEY,
    })(request()));
    assert.equal(projectSafeMemoryV3DialogueTransportDiagnostic(revoked), "transport_failed");
  });
});
