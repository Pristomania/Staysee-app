...
export interface MemoryV3DialogueTransportResult {
  rawContent: string;
  usage: {
    promptTokens: number;
...
export type MemoryV3DialogueModelAdapter = (
  request: MemoryV3DialogueReconcileRequest,
) => Promise<MemoryV3DialogueTransportResult>;

export interface MemoryV3DialogueOpenRouterAdapterOptions {
  apiKey: string;
...
const DIALOGUE_RESPONSE_SCHEMA = deepFreeze({
  type: "object",
...
function makeError(token: object, message: string, diagnosticCode?: DiagnosticCode): Error {
  const error = new Error(`[memory-v3:dialogue-transport] ${message}`);
  error.name = "MemoryV3DialogueTransportError";
...
export function projectSafeMemoryV3DialogueTransportDiagnostic(error: unknown): DiagnosticCode | null {
...
function projectRequest(token: object, unsafeRequest: unknown): MemoryV3DialogueReconcileRequest {
  const outer = inspectRecord(token, unsafeRequest, ["system", "input"]);
  if (outer.system !== MEMORY_V3_DIALOGUE_SYSTEM_INSTRUCTION) fail(token, "invalid input");
  const input = inspectRecord(token, outer.input, [
    "schemaVersion", "userLanguage", "session", "messages", "currentItems", "candidates",
  ]);
  if (input.schemaVersion !== "memory-v3-dialogue-reconcile-request-v1" || input.userLanguage !== "ru") {
...
function projectUsage(token: object, value: unknown): MemoryV3DialogueTransportResult["usage"] {
...
function projectProviderBody(token: object, value: unknown): MemoryV3DialogueTransportResult {
...
export function createMemoryV3DialogueOpenRouterAdapter(
  options: MemoryV3DialogueOpenRouterAdapterOptions,
  ...
  return async (unsafeRequest: MemoryV3DialogueReconcileRequest) => {
    const token = {};
    try {
        ...
        const body = {
          ...
          response_format: {
            ...
            json_schema: {
              ...
              strict: true,
              schema: structuredClone(DIALOGUE_RESPONSE_SCHEMA),
            },
          },
