...
import type { MemoryV3LifecycleState } from "./lifecycleContract.ts";
import {
...
const RAW_SENTINEL = "RAW_LIFECYCLE_PROMPT_SECRET_SENTINEL";

const EXPECTED_SYSTEM = `You reconcile validated StaySEE Memory V3 candidates with existing lifecycle memory.
...
function state(): MemoryV3LifecycleState {
  return {
...
function extraction(): MemoryV3Extraction {
  return {
    run: { caseId: "lifecycle-step-01", extractorVersion: "fixture-v1" },
    items: [
      {
...
function build(overrides: Record<string, unknown> = {}) {
  return buildMemoryV3LifecycleReconcileRequest({
    userId: USER_ID,
...
describe("Memory V3 lifecycle reconciler projection", () => {
  ...
  it("does not leak trusted identities, gold, legacy, database, or provider fields", () => {
    ...
    for (const forbidden of [
      ...
      "firstSeenAt", "updatedAt", "conversationId\":null", "provider", "usage", "Authorization",
  ...
  it("copies inputs without mutation or aliasing and keeps dialogue injection in message text", () => {
    ...
    const bundle = buildMemoryV3LifecycleReconcileRequest(input);
...
describe("Memory V3 lifecycle reconciler boundary", () => {
