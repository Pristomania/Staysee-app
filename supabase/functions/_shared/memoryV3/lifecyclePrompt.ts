...
import {
  type MemoryV3LifecycleReferenceBindings,
...
function makeError(token: object): Error {
  const error = new Error("[memory-v3:lifecycle-prompt] invalid input");
  error.name = "MemoryV3LifecyclePromptError";
...
function evidenceIdentity(row: MemoryV3LifecycleProjectedEvidence): string {
...
function projectedEvidence(rows: Array<{
  ...
  episodeKey: string | null;
  mentionTime: string;
}>): MemoryV3LifecycleProjectedEvidence[] {
  return rows.map((row) => ({
...
export function buildMemoryV3LifecycleReconcileRequest(input: {
  ...
  return boundary((token) => {
    ...
    const state = validateMemoryV3LifecycleState(cloneJsonData(token, projected.state), userId);
    ...
    const sortedCandidates = Array.isArray(extraction?.items)
      ...
      : fail(token);
    const bindings: MemoryV3LifecycleReferenceBindings = {
    ...
    const currentItems: MemoryV3LifecycleProjectedItem[] = sortedItems.map((item) => ({
    ...
    const candidates: MemoryV3LifecycleProjectedCandidate[] = sortedCandidates.map((item) => ({
    ...
    const request: MemoryV3LifecycleReconcileRequest = {
