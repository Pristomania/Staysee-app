...
import {
  ...
  type MemoryV3LifecycleEvidence,
  type MemoryV3LifecycleItem,
...
const EMPTY_FIELDS = ["userId"] as const;
const STEP_FIELDS = [
  "state",
  "at",
  "conversationId",
...
const OPERATION_RANK: Record<MemoryV3LifecycleOperationType, number> = {
  create: 0,
  confirm: 1,
  revise: 2,
  mark_stale: 3,
...
const KEY_NAMESPACE = "memory-v3-production-lifecycle-v1";

function makeError(token: object, diagnosticCode: Diagnostic): Error {
  const error = new Error("[memory-v3:lifecycle-reducer] value is invalid");
...
function inspectRecord(
  ...
  try {
    ...
    if (typeof value !== "object" || value === null) fail(token, "lifecycle_reducer_invalid_input");
  ...
  for (const key of keys) {
    if (typeof key !== "string" || !allowed.has(key)) fail(token, "lifecycle_reducer_invalid_input");
  ...
  for (const field of fields) {
    if (!Object.hasOwn(output, field)) fail(token, "lifecycle_reducer_invalid_input");
  }
  return output;
}
...
function cloneJsonData(token: object, value: unknown, seen = new WeakSet<object>()): unknown {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value)) fail(token, "lifecycle_reducer_invalid_input");
    return value;
  }
  if (typeof value !== "object") fail(token, "lifecycle_reducer_invalid_input");
  if (seen.has(value)) fail(token, "lifecycle_reducer_invalid_input");
  seen.add(value);
  let isArray: boolean;
  let prototype: object | null;
  let keys: PropertyKey[];
  ...
  if (isArray) {
    ...
    if (keys.length !== length + 1) fail(token, "lifecycle_reducer_invalid_input");
  }
  ...
  if (isArray && output.length !== length) fail(token, "lifecycle_reducer_invalid_input");
  seen.delete(value);
  return output;
}
...
function evidenceIdentity(row: MemoryV3LifecycleEvidence): string {
...
function mergeEvidence(
  token: object,
  existing: MemoryV3LifecycleEvidence[],
  incoming: MemoryV3LifecycleEvidence[],
): MemoryV3LifecycleEvidence[] {
...
function cloneStateItem(item: MemoryV3LifecycleItem): MemoryV3LifecycleItem {
  return { ...item, evidence: item.evidence.map((row) => ({ ...row })) };
}

function canonicalStateForComparison(state: MemoryV3LifecycleState): MemoryV3LifecycleState {
  return {
    ...state,
...
function validateInternalProposal(
  ...
  if (!Array.isArray(cloned)) fail(token, "lifecycle_reducer_invalid_input");
...
function operationComparator(
  left: MemoryV3LifecycleProposal[number],
  right: MemoryV3LifecycleProposal[number],
): number {
...
export function createEmptyMemoryV3LifecycleState(input: { userId: string }): MemoryV3LifecycleState {
  return boundary("lifecycle_reducer_invalid_input", (token) => {
    ...
    const state: MemoryV3LifecycleState = {
    ...
    return deepFreeze(validateMemoryV3LifecycleState(state, projected.userId));
  });
}
...
export async function applyMemoryV3LifecycleStep(input: {
  ...
  return asyncBoundary("lifecycle_reducer_invalid_input", async (token) => {
    ...
    if (!validDateTime(projected.at) || typeof projected.conversationId !== "string" ||
        projected.conversationId.trim().length === 0) fail(token, "lifecycle_reducer_invalid_input");
    ...
    const expectedUserId = typeof stateCopy === "object" && stateCopy !== null
      ...
      : undefined;
    if (typeof expectedUserId !== "string") fail(token, "lifecycle_reducer_invalid_input");
    const originalState = validateMemoryV3LifecycleState(stateCopy, expectedUserId);
    const extraction = cloneJsonData(token, projected.extraction) as MemoryV3Extraction;
    const forgetKeys = validateMemoryV3TrustedForgetKeys(
      cloneJsonData(token, projected.trustedForgetMemoryKeys),
      originalState,
    );
    const forgotten = new Set(forgetKeys);
    const working: MemoryV3LifecycleState = {
      schemaVersion: MEMORY_V3_LIFECYCLE_SCHEMA_VERSION,
    ...
    let intermediate: MemoryV3LifecycleState;
    try {
      intermediate = validateMemoryV3LifecycleState(working, originalState.userId);
    } catch {
      fail(token, "lifecycle_reducer_transition_invalid");
    }
    ...
    try {
      state = validateMemoryV3LifecycleState(working, originalState.userId);
    } catch {
      fail(token, "lifecycle_reducer_transition_invalid");
    }
...
export function projectSafeMemoryV3LifecycleReducerDiagnostic(error: unknown): Diagnostic | null {
