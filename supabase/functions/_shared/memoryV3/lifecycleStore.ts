...
const DIAGNOSTICS = new Set<MemoryV3LifecycleStoredDiagnostic>([
...
interface InspectedClient { target: object; rpc: MemoryV3LifecycleRpcClient["rpc"] }

function fail(): Error {
  const error = new Error("[memory-v3:lifecycle-store] operation failed");
  error.name = "MemoryV3LifecycleStoreError";
...
function inspectClient(value: unknown): InspectedClient {
  ...
  while (current !== null) {
    ...
    if (own) {
      ...
      return { target: value, rpc: own.value as MemoryV3LifecycleRpcClient["rpc"] };
    }
...
function projectState(value: unknown, userId: string): MemoryV3LifecycleState {
  const cloned = cloneJson(value);
  try { return validateMemoryV3LifecycleState(cloned, userId); } catch { throw fail(); }
}
...
function projectExtraction(value: unknown, userId: string): MemoryV3Extraction {
  ...
  const items = array(root.items, MEMORY_V3_LIFECYCLE_MAX_CANDIDATES).map((raw) => {
    ...
    if (typeof item.localItemKey !== "string" || !SHA256.test(item.localItemKey) || !KINDS.has(item.kind as string) || !nonEmpty(item.claim) || item.scope !== "cross_conversation" || item.conversationId !== null || typeof item.status !== "string" || !STATUS[item.kind as string].has(item.status) || (item.sensitivity !== "normal" && item.sensitivity !== "sensitive")) throw fail();
  ...
  const evidence = array(root.evidence, MEMORY_V3_LIFECYCLE_MAX_CANDIDATE_EVIDENCE).map((raw) => {
  ...
  return { run: { caseId: run.caseId, extractorVersion: run.extractorVersion }, items, evidence } as MemoryV3Extraction;
}
...
function projectTransitions(value: unknown): MemoryV3LifecycleSuccessWrite["transitions"] {
  return array(value, MEMORY_V3_LIFECYCLE_MAX_CANDIDATES + 100).map((raw) => {
    ...
    return { ...transition } as MemoryV3LifecycleSuccessWrite["transitions"][number];
  });
}

function projectUsage(value: unknown): MemoryV3LifecycleUsage | null {
  ...
  return usage as unknown as MemoryV3LifecycleUsage;
}

export function createMemoryV3LifecycleStore(clientValue: MemoryV3LifecycleRpcClient): MemoryV3LifecycleStore {
  ...
  return {
    async reserve(inputValue) {
      ...
      const state = projectState(row.state, input.userId as string);
    ...
    async compareAndSwap(inputValue) {
      const input = record(inputValue, ["runId", "userId", "expectedStateRevision", "state", "changed", "extraction", "operations", "transitions", "extractorUsage", "reconcilerUsage"]);
      if (typeof input.runId !== "string" || !UUID.test(input.runId) || typeof input.userId !== "string" || !UUID.test(input.userId) || !Number.isSafeInteger(input.expectedStateRevision) || (input.expectedStateRevision as number) < 0 || typeof input.changed !== "boolean") throw fail();
      const state = projectState(input.state, input.userId as string);
      ...
      const extraction = projectExtraction(input.extraction, input.userId as string);
      ...
      const data = await callRpc(client, "apply_memory_v3_lifecycle_shadow_state", {
        p_run_id: input.runId, p_user_id: input.userId, p_expected_state_revision: input.expectedStateRevision,
