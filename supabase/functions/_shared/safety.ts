...
const CRISIS_RESPONSE = `Сейчас важно не оставаться с этим одной.
...
`.trim(),
  ...
  off_topic: `
УКАЗАНИЕ БЕЗОПАСНОСТИ (не показывать пользователю):
Запрос не про её переживание, а про услугу (текст, план, код, перевод, задача «сделай за меня»).
НЕ выполняй задание. НЕ пиши длинный материал. 2–3 предложения: коротко отрази, что она просит, и мягко верни к себе — что за этим стоит, что она чувствует, что ей важно.
...
Признай, как тяжело и как сильно хочется ответа «сейчас». 2–4 предложения: тёплая твёрдость + StaySee-выход (что за этим страхом; один её шаг; не исполнение поручения).
