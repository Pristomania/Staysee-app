-- Memory V3 lifecycle shadow: normalized durable state, CAS, source deletion, and audit retention.

CREATE TABLE public.memory_v3_lifecycle_shadow_heads (
  user_id uuid PRIMARY KEY REFERENCES public.profiles(id) ON DELETE CASCADE,
  schema_version text NOT NULL DEFAULT 'memory-v3-lifecycle-state-v1'
    CHECK (schema_version = 'memory-v3-lifecycle-state-v1'),
...
CREATE TABLE public.memory_v3_lifecycle_shadow_items (
  ...
  FOREIGN KEY (user_id) REFERENCES public.memory_v3_lifecycle_shadow_heads(user_id) ON DELETE CASCADE,
  CHECK (
...
CREATE TABLE public.memory_v3_lifecycle_shadow_evidence (
  user_id uuid NOT NULL,
  ...
  PRIMARY KEY (user_id, memory_key, source_message_id, relation),
  FOREIGN KEY (user_id, memory_key)
    REFERENCES public.memory_v3_lifecycle_shadow_items(user_id, memory_key) ON DELETE CASCADE,
...
CREATE TABLE public.memory_v3_lifecycle_shadow_identities (
...
CREATE TABLE public.memory_v3_lifecycle_shadow_runs (
  ...
  FOREIGN KEY (user_id, conversation_id, pipeline_version, input_hash)
    REFERENCES public.memory_v3_lifecycle_shadow_identities(user_id, conversation_id, pipeline_version, input_hash)
    ON DELETE CASCADE,
  CONSTRAINT memory_v3_lifecycle_shadow_runs_extractor_usage_shape CHECK (
  ...
  CONSTRAINT memory_v3_lifecycle_shadow_runs_reconciler_usage_shape CHECK (
  ...
  CONSTRAINT memory_v3_lifecycle_shadow_runs_terminal_shape CHECK (
...
CREATE INDEX memory_v3_lifecycle_shadow_evidence_source_idx
  ON public.memory_v3_lifecycle_shadow_evidence(source_message_id);
CREATE INDEX memory_v3_lifecycle_shadow_evidence_conversation_idx
  ON public.memory_v3_lifecycle_shadow_evidence(conversation_id);
CREATE INDEX memory_v3_lifecycle_shadow_runs_user_created_idx
  ON public.memory_v3_lifecycle_shadow_runs(user_id, created_at DESC);
CREATE INDEX memory_v3_lifecycle_shadow_runs_conversation_created_idx
  ON public.memory_v3_lifecycle_shadow_runs(conversation_id, created_at DESC);
CREATE INDEX memory_v3_lifecycle_shadow_runs_created_idx
  ON public.memory_v3_lifecycle_shadow_runs(created_at);

ALTER TABLE public.memory_v3_lifecycle_shadow_heads ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.memory_v3_lifecycle_shadow_items ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.memory_v3_lifecycle_shadow_evidence ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.memory_v3_lifecycle_shadow_identities ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.memory_v3_lifecycle_shadow_runs ENABLE ROW LEVEL SECURITY;
...
CREATE OR REPLACE FUNCTION public.fail_memory_v3_lifecycle_shadow_run(
...
CREATE OR REPLACE FUNCTION public.purge_memory_v3_lifecycle_shadow_runs()
RETURNS bigint LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $$
DECLARE v_deleted bigint;
BEGIN
  DELETE FROM public.memory_v3_lifecycle_shadow_runs
...
CREATE TRIGGER delete_memory_v3_lifecycle_items_before_message
BEFORE DELETE ON public.messages FOR EACH ROW
EXECUTE FUNCTION public.delete_memory_v3_lifecycle_items_for_message();
CREATE TRIGGER delete_memory_v3_lifecycle_items_before_conversation
BEFORE DELETE ON public.conversations FOR EACH ROW
EXECUTE FUNCTION public.delete_memory_v3_lifecycle_items_for_conversation();

REVOKE ALL ON TABLE public.memory_v3_lifecycle_shadow_heads FROM PUBLIC, anon, authenticated;
REVOKE ALL ON TABLE public.memory_v3_lifecycle_shadow_items FROM PUBLIC, anon, authenticated;
REVOKE ALL ON TABLE public.memory_v3_lifecycle_shadow_evidence FROM PUBLIC, anon, authenticated;
REVOKE ALL ON TABLE public.memory_v3_lifecycle_shadow_identities FROM PUBLIC, anon, authenticated;
REVOKE ALL ON TABLE public.memory_v3_lifecycle_shadow_runs FROM PUBLIC, anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.memory_v3_lifecycle_shadow_heads TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.memory_v3_lifecycle_shadow_items TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.memory_v3_lifecycle_shadow_evidence TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.memory_v3_lifecycle_shadow_identities TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.memory_v3_lifecycle_shadow_runs TO service_role;
...
REVOKE ALL ON FUNCTION public.fail_memory_v3_lifecycle_shadow_run(uuid, uuid, text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.apply_memory_v3_lifecycle_shadow_state(uuid, uuid, bigint, jsonb, boolean, jsonb, jsonb, jsonb, jsonb, jsonb) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.purge_memory_v3_lifecycle_shadow_runs() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.delete_memory_v3_lifecycle_items_for_message() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.delete_memory_v3_lifecycle_items_for_conversation() FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION public.fail_memory_v3_lifecycle_shadow_run(uuid, uuid, text) TO service_role;
GRANT EXECUTE ON FUNCTION public.apply_memory_v3_lifecycle_shadow_state(uuid, uuid, bigint, jsonb, boolean, jsonb, jsonb, jsonb, jsonb, jsonb) TO service_role;
GRANT EXECUTE ON FUNCTION public.purge_memory_v3_lifecycle_shadow_runs() TO service_role;

SELECT cron.unschedule('memory-v3-lifecycle-shadow-purge-daily')
WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'memory-v3-lifecycle-shadow-purge-daily');
SELECT cron.schedule(
  'memory-v3-lifecycle-shadow-purge-daily',
  '29 3 * * *',
  $cron$SELECT public.purge_memory_v3_lifecycle_shadow_runs();$cron$
);
