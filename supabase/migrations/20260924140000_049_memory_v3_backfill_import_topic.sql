-- Fixes a real gap found while implementing Task 7b of the topic-
-- classification feature: import_memory_v3_lifecycle_backfill_state
-- (originally defined in 037_memory_v3_lifecycle_backfill_import.sql) is
-- the entry point for a one-time historical backfill import -- it
-- independently re-validates the whole reviewed state JSON (mirroring
-- validateMemoryV3LifecycleState) before writing it, and that
-- independent item-shape check still expected exactly the pre-topic
-- twelve keys, with no allowance for 'topic' at all. Since topic became
-- a required key on every persisted lifecycle item, any real import
-- built from the live reducer's output would be rejected outright by
-- this stricter shape check, and even if the check were loosened without
-- also touching the INSERT, the classified topics -- paid for through the
-- real reconciler pipeline -- would be silently discarded, writing NULL
-- rows. Transcribed from the CURRENT live version of the function (it has
-- not been superseded by any later migration), with 'topic' added to the
-- item-shape validation (key count, allowed-key list, and a null-or-enum
-- value check placed right after 'alternative', matching the production
-- field order in lifecycleContract.ts's ITEM_FIELDS) and to the item
-- INSERT's column list and value list, and every other line -- including
-- all inline comments -- kept byte-identical to that source migration.
...
BEGIN
    ...
    WHERE pg_catalog.jsonb_typeof(item) IS DISTINCT FROM 'object'
      ...
      OR (
        ...
        OR (pg_catalog.jsonb_typeof(item->'topic') = 'string'
          AND item->>'topic' IN ('life_context', 'communication', 'preference'))
      ) IS DISTINCT FROM true
      OR (
  ...
  INSERT INTO public.memory_v3_lifecycle_shadow_items(
    ...
    event_time_end, alternative, topic, first_seen_at, updated_at, revision
  )
